...
public class PannableZoomableViewModified extends Application {
  ...
  public PannableZoomableViewModified() {
  ...
  public void start(final Stage stage) {
    ...
    group.getChildren().add(new ImageView(new Image("https://www.narniaweb.com/wp-content/uploads/2009/08/NarniaMap.jpg", true)));
  ...
  private Parent createZoomPane(final Group group) {
    ...
    scrollContent.setOnMousePressed((MouseEvent event) -> lastMouseCoordinates.set(new Point2D(event.getX(), event.getY())));

    scrollContent.setOnMouseDragged((MouseEvent event) -> {
      double scrollerHvalue = Double.isNaN(scroller.getHvalue()) ? 0 : scroller.getHvalue();
      double scrollerVvalue = Double.isNaN(scroller.getVvalue()) ? 0 : scroller.getVvalue();

      double deltaX = event.getX() - lastMouseCoordinates.get().getX();
      double extraWidth = scrollContent.getLayoutBounds().getWidth() - scroller.getViewportBounds().getWidth();
      double deltaH = deltaX * (scroller.getHmax() - scroller.getHmin()) / extraWidth;
      double desiredH = scrollerHvalue - deltaH;
      scroller.setHvalue(Math.max(0, Math.min(scroller.getHmax(), desiredH)));

      double deltaY = event.getY() - lastMouseCoordinates.get().getY();
      double extraHeight = scrollContent.getLayoutBounds().getHeight() - scroller.getViewportBounds().getHeight();
      double deltaV = deltaY * (scroller.getVmax() - scroller.getVmin()) / extraHeight;
      double desiredV = scrollerVvalue - deltaV;
  ...
  private Point2D figureScrollOffset(Node scrollContent, ScrollPane scroller) {
    ...
    double extraWidth = scrollContent.getLayoutBounds().getWidth() - scroller.getViewportBounds().getWidth();
    double hScrollProportion = (scrollerHvalue - scroller.getHmin()) / (scroller.getHmax() - scroller.getHmin());
    double scrollXOffset = hScrollProportion * Math.max(0, extraWidth);
    double extraHeight = scrollContent.getLayoutBounds().getHeight() - scroller.getViewportBounds().getHeight();
    double vScrollProportion = (scrollerVvalue - scroller.getVmin()) / (scroller.getVmax() - scroller.getVmin());
  ...
  private SVGPath createCurve() {
    ...
    ellipticalArc.setStrokeWidth(DEFAULT_STROKE_WIDTH);
  ...
  private SVGPath createStar() {
    ...
    star.setStrokeWidth(DEFAULT_STROKE_WIDTH);
    return star;
  }
  ...
  private MenuBar createMenuBar(final Stage stage, final Group group) {
    ...
    zoomResetMenuItem.setOnAction((ActionEvent event) -> {
      group.setScaleX(1);
      group.setScaleY(1);
    });
    ...
    zoomInMenuItem.setOnAction((ActionEvent event) -> {
      group.setScaleX(group.getScaleX() * DEFAULT_ZOOM_FACTOR);
      group.setScaleY(group.getScaleY() * DEFAULT_ZOOM_FACTOR);
    });
    MenuItem zoomOutMenuItem = new MenuItem("Zoom _Out");
    zoomOutMenuItem.setAccelerator(new KeyCodeCombination(KeyCode.O));
    zoomOutMenuItem.setGraphic(new ImageView(new Image(ZOOM_OUT_ICON)));
    zoomOutMenuItem.setOnAction((ActionEvent event) -> {
      group.setScaleX(group.getScaleX() * 1 / DEFAULT_ZOOM_FACTOR);
      group.setScaleY(group.getScaleY() * 1 / DEFAULT_ZOOM_FACTOR);
    });
